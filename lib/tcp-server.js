...
module.exports = function serverPlugin (upring, opts, next) {
  ...
  function handler (stream) {
    ...
    upring.logger.debug({ address: stream.address() }, 'incoming connection')

    const instance = tentacoli()
    upring._inbound.add(instance)
    pump(stream, instance, stream, () => {
      upring.logger.debug({ address: stream.address() }, 'closed connection')
  ...
  function onListen () {
    upring.logger.debug({ address: upring._server.address() }, 'listening')
    next()
  }
}
